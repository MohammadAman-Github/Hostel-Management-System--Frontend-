...
export const createMonthlyRentInDb = async (
  ...
  if (previousMrd) {
    ...
    lastReading =
      Number(previousMrd.currentReading) ||
      lastReading
  ...
  if (
    mrdData.totalRentPaid !== undefined
  ) {
...
export const updateMRDInDb = async (
  ...
  await db.run(
    ...
    `,
    [
      Number(existing.currentReading),
      arrearBill,
      Number(roomNo)
    ]
  )
...
export const deleteMRDInDb = async (
  ...
  return {
    message:
      `Monthly rent for Room ${roomNo}, ${month} ${year} deleted successfully.`
  }
}